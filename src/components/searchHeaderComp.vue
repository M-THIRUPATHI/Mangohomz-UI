<template>
  ...
  <div class="row q-pa-xs lt-sm">
    ...
    >
      ...
      <q-select
        ...
        @input="loadAllHospitals(searchItem.city);emptyHospitals();selectCities(searchItem.city)"
      >
      </q-select>
      <q-select
  ...
  <!--Website view Start-->
    <div class="row gt-xs" style="padding-top: 0px;" >
      ...
      <q-btn class="text-white bg-green" style="margin-left: 15px" aria-label="Mangohomz Booking Search Button" flat icon="search"  @click="loadSearchResults" />
    </div>
    <!--Website View End-->
  </q-page-container>
      ...
      selectCities(item){
       ...
       goToNext() {
        if(this.searchItem.city) {
          this.searchItem.guests = this.searchItem.adult_count + this.searchItem.child_count;
