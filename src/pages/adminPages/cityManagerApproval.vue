          ...
          <q-layout view="Lhh lpR fff" container class="bg-white">
            <q-header class="bg-primary">
              <q-bar class="bg-primary text-white">
                <q-icon name="feed" />
                <div class="text-h6">
                  City Manager Registration Details: {{ editedItem.first_name }}
                </div>
                <q-space />
...
export default {
  data() {
    return {
      title: "City Manager Registration Approvals",
      selected: [],
      data: [],
      editedIndex: -1,
      details: false,
  ...
  mounted() {
    this.$store.dispatch("account/getCityManagerRegistrationData");
  },
  computed: {
    ...mapState({
      userDetails() {
      ...
      ...mapGetters("account", ["city_reg_rows"]),
      partner_reg_columns: state => state.account.partner_reg_columns
    })
  },
  methods: {
    ...mapActions("account", ["approveCityManager"]),
    ...mapActions("account", ["rejectCityManager"]),

  confirm() {
      this.$q
        .dialog({
          title: "Confirm",
          message: "Would you like to Approve For City Manager Registration ?",
          cancel: true,
          persistent: true
        })
        .onOk(() => {
          this.approveCityManager(this.editedItem);
          this.details = false;
        })
        .onCancel(() => {
          this.details = true;
    ...
    reject() {
      this.$q
        .dialog({
          title: "Confirm",
          message: "Do you want to Reject City Manager Registration ?",
          cancel: true,
          persistent: true
        })
        .onOk(() => {
          this.rejectCityManager(this.editedItem);
          this.details = false;
        })
        .onCancel(() => {
          this.details = true;
