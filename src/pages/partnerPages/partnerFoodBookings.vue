    ...
    <q-page style="padding-top: 130px" class="q-pa-md">
      <div class="q-pa-md" style="padding-top: 20px">
        <q-table
          ...
          :title="title"
          :data="foodPartner_bkng_rows"
          :columns="foodPartner_bkng_columns"
...
export default {
  data() {
    return {
      ...
      bookingDialog: false,
      title: "Food Booking Details of Customer",
      mode: "list",
      filter: "",
      addDataDialog: false,
      statesList: [],
  ...
  computed: {
    ...mapState({
      ...
      foodPartner_bkng_columns: state => state.booking.foodPartner_bkng_columns,
      foodItem_bkng_columns: state => state.booking.foodItem_bkng_columns
    }),

    ...mapGetters("booking", ["foodPartner_bkng_rows"])
  },
  mounted() {
    this.$store.dispatch("booking/guestFoodBookingData");
  },
  meta() {
    return {
      title: this.title
  ...
  methods: {
    ...
    exportTable() {
      ...
      const content = [
        this.foodPartner_bkng_columns.map(col => wrapCsvValue(col.label))
      ]
        .concat(
          this.foodPartner_bkng_rows.map(row =>
            this.foodPartner_bkng_columns
              .map(col =>
                wrapCsvValue(
