    ...
    <q-page style="padding-top: 130px" class="q-pa-md">
      <div class="q-pa-md" style="padding-top: 20px">
        <q-table
          ...
          :title="title"
          :data="travelPartner_bkng_rows"
          :columns="travelPartner_bkng_columns"
...
export default {
  data() {
    return {
      ...
      bookingDialog: false,
      title: "Travel Booking Details of Customer",
      mode: "list",
      filter: "",
      addDataDialog: false,
      statesList: [],
  ...
  computed: {
    ...mapState({
      ...
      travelPartner_bkng_columns: state =>
        state.booking.travelPartner_bkng_columns,
      travelItem_bkng_columns: state => state.booking.travelItem_bkng_columns
    }),

    ...mapGetters("booking", ["travelPartner_bkng_rows"])
  },
  mounted() {
    this.$store.dispatch("booking/guestTravelBookingData");
  },
  meta() {
    return {
      title: this.title
  ...
  methods: {
    ...
    exportTable() {
      ...
      const content = [
        this.travelPartner_bkng_columns.map(col => wrapCsvValue(col.label))
      ]
        .concat(
          this.travelPartner_bkng_rows.map(row =>
            this.travelPartner_bkng_columns
              .map(col =>
                wrapCsvValue(
